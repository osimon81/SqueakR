...
We can also plot the call clusters (custom labels) on a 3D plane to examine the density of calls as a function of principal frequency, call length, and mean power (dB/kHz) below:
...
## Inter-group Summary [Development Version]
...
```{r SqueakR dashboard, eval = FALSE}
squeakRDashboard()
```
...
This function is currently available in the GitHub release of this package, but not the CRAN version yet.

Currently, the SqueakR Shiny Dashboard is a separate, non-CRAN repository which is downloaded independently of the SqueakR package. Consolidating the dashboard and package under a single CRAN package is a current aim of this package, where the Dashboard can be run as a separate function within the SqueakR package. The most recent build of SqueakR on GitHub features the dashboard embedded in the package as a function `squeakRDashboard()`, yet these changes have yet to be reflected on the CRAN release. At the point of the merge on CRAN, the SqueakR Dashboard repository on GitHub is planned to be deprecated.
